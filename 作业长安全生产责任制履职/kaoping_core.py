#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""作业长履职考评表生成核心逻辑 v3.0.0（兼容 Windows 7 SP1 ~ Windows 11）"""
...
TEMPLATE_NAME = "作业长安全生产责任制履职履责考评表（模板）.doc"


# 考评项 → 表格主行映射：13 个考评项，每项一个主填写行
# （各主行的 C6~C10 与续行纵向合并，填入主行即可覆盖整个考评块；续行 C4/C5 为模板固定内容）。
ITEM_ROWS = {
    1: 3,     # 安全绩效（20，占 R3+R4）
    2: 5,     # 隐患排查（5，占 R5+R6）
    3: 7,     # 安全检查（5）
    4: 8,     # 安全教育（5）
    5: 9,     # 协力安全（5，占 R9+R10）
    6: 11,    # 危险源管理（5，占 R11~R13）
    7: 14,    # 危化品（5）
    8: 15,    # 应急管理（5，占 R15+R16）
    9: 17,    # 违章查处（20，占 R17+R18）
    10: 19,   # 工机具（5）
    11: 20,   # 劳防用品（5）
    12: 21,   # 履职评价（5，占 R21~R23）
    13: 24,   # “六个一”工作实绩（10，占 R24~R29，C6~C8 合并，C9/C10 逐行独立）
}
TOTAL_ROW = 30
...
DEFAULT_NAME_PATTERN = "作业长安全生产责任制履职履责考评表({X}月{XXX}).doc"
...
# 关键词基于作业长 13 个考评项标题与常见当月文件夹命名归纳（首词权重 2，可编辑 material_rules.json）。
ITEM_MATCH_RULES = {
    1: ["安全绩效", "绩效", "事故", "工伤", "轻伤", "险肇", "目标", "否决"],
    2: ["隐患排查", "隐患", "排查", "登记", "整改", "扫雷", "提报"],
    3: ["安全检查", "检查", "综合检查", "巡查", "督查", "整改"],
    4: ["安全教育", "培训", "教育", "学习", "双考", "资质", "考试", "授课"],
    5: ["协力安全", "协力", "交底", "区域", "作业工票", "工票", "清底"],
    6: ["危险源管理", "危险源", "风险", "辨识", "控制点", "评估", "绩效评审"],
    7: ["危化品", "危化品", "化学品", "危险化学品", "一品一表", "消防器材"],
    8: ["应急管理", "应急", "演练", "突发", "处置", "预案", "报送", "排水"],
    9: ["违章查处", "违章", "查处", "三违", "违规", "违纪", "记分", "禁令"],
    10: ["工机具", "工机具", "机具", "工具", "设备", "设施", "维护", "翻斗车"],
    11: ["劳防用品", "劳防", "劳保", "防护", "职业卫生", "粉尘", "旁站", "监护"],
    12: ["履职评价", "履职", "责任书", "考评", "挂钩点", "安全活动", "计划"],
    13: ["六个一", "六个一", "安全专题会", "风险评审", "交心", "改善项目",
         "案例", "行为观察", "隐患排查", "授课", "实战演练"],
}
...
# ============ 月度履职履责表(xlsx) 评分自动读取 ============
# 目标 sheet：「作业长月度履职评价表」（关键字匹配，排除「评价标准」）。
# 布局参照安全员版实测规律：表头行为含「扣分说明」的行；偶数列为各考评项名、奇数列为其扣分说明；
# 姓名在「姓名」列；得分合计在含「得分」「合计」的表头列。
# 注：作业长履职履责表实际版式需以业务方提供的月度 xlsx 为准（关键词匹配自适应）。
XLSX_SHEET_INCLUDE = ("作业长", "评价")
...
def _locate_eval_sheet(xlsx_path):
    ...
    for sn in wb.sheetnames:
        if (all(k in sn for k in XLSX_SHEET_INCLUDE)
            ...
            break
    if sheet is None:
        raise ValueError("未找到「作业长月度履职评价表」sheet，可用：%s"
                         % "、".join(wb.sheetnames))
...
def _fill_header(cell, name, month):
    ...
    # 姓名：考评对象（作业长）：...管理者姓名
    body = _replace_slot(body, "考评对象（作业长）：", "管理者姓名", str(name).strip())
    # 月份：评价月份：...评价人员签字
...
def generate_doc(template_path, output_path, name, month, items, year=None,
    ...
    try:
        ...
        if "考评对象" not in header or "评价月份" not in header:
            raise ValueError(
                "模板表头缺少「考评对象（作业长）」或「评价月份」标签，请更换正确模板"
            )
