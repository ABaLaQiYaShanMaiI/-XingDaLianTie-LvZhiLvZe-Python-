#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""作业长安全生产责任制履职履责考评表 自动生成工具 GUI v3.0.0（兼容 Windows 7 SP1 ~ Windows 11）"""
import logging
import os
import sys
import json
...
# 双行考评项：作业长模板无双行独立填写项（各考评项 C6~C10 与续行纵向合并，填主行即可），保持空。
SUB_ROW_MAX = {}
...
class App:
    def __init__(self, root):
        ...
        self.items_widgets = []
        root.title(f"作业长安全生产责任制履职履责考评表自动生成工具 v{VERSION}")
    ...
    def _pick_eval_name(self, names):
        ...
        ttk.Label(win, text="履职履责表中包含多个作业长，请选择本次生成的姓名：",
                  padding=(14, 10)).pack()
