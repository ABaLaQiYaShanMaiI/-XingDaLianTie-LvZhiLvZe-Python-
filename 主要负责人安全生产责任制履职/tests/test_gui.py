...
def test_material_rules_and_weights():
    ...
    try:
        ...
        assert kc.match_materials_file("六个一/安全专题会纪要.doc") == 12
...
def test_apply_kaoping_doc_fill():
    ...
    tmp = tempfile.mkdtemp()
    try:
        fake_doc = os.path.join(tmp, "汪庆7月.doc")
        open(fake_doc, "w").close()
        fake_mat = _touch(tmp, "材料", "证据.xlsx")
        fake_data = {
            "name": "汪庆", "month": "7", "warnings": ["某警告"],
        ...
        fake_data["items"][7] = {"desc": "未按违章查处", "score": "8",
                                 "material_text": "", "materials": [fake_mat],
                                 "eval_desc": "未按", "super_score": "8"}
        ...
        try:
            ...
            try:
                ...
                assert app.name_var.get() == "汪庆"
                assert app.month_var.get() == "7"
                it1 = app.items_widgets[0]
                assert it1.desc_var.get() == "无事故"
                assert it1.score_var.get() == "20" and it1.super_var.get() == "20"
                it7 = app.items_widgets[6]
                assert it7.score_var.get() == "8"
                assert it7.material_paths == [fake_mat]
...
def test_eval_scores_fill():
    ...
    try:
        ...
        kc.list_eval_names = lambda p: ["汪庆", "王旺"]
        ...
        try:
            ...
            try:
                ...
                assert list(app.name_combo["values"]) == ["汪庆", "王旺"]
