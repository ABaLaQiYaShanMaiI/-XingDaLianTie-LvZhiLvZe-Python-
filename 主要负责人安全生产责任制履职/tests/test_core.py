...
def test_replace_slot():
    body = "考评对象（主要负责人）：  孙忠      管理者姓名：x评价月份：                 评价人员签字"
    b = kc._replace_slot(body, "考评对象（主要负责人）：", "管理者姓名", "测试员")
...
def test_item_rows():
    ...
    assert kc._item_rows(2) == [6]
    assert kc._item_rows(7) == [16]
    assert kc._item_rows(9) == [20]
    assert kc._item_rows(13) == [40]
    assert kc.TOTAL_ROW == 41
...
def test_match_materials_file():
    ...
    assert kc.match_materials_file("（炼铁）8月主题工作/1.安委会纪要.doc") == 11
    assert kc.match_materials_file("应急演练/皮带机演练方案.doc") == 6
    assert kc.match_materials_file("危险源/危险源控制点自查表.xlsx") == 5
    assert kc.match_materials_file("安全培训课件.pdf") == 4
    assert kc.match_materials_file("违章查处记录.xls") == 7
    assert kc.match_materials_file("制度/岗位规程修订.doc") == 9
    # 无匹配
...
def test_scan_materials_folder():
    ...
    try:
        ...
        # 应被跳过的考评表文件
        open(os.path.join(tmp, "主要负责人安全生产责任制履职清单考评表（张三8月）.doc"), "w").close()
        # 应被跳过的临时文件
...
def _make_fake_lvzhilv():
    ...
    ws = wb.active
    ws.title = "主要负责人月度履职评价表"
    ws.cell(1, 1, "主要负责人（事业部）月度安全生产履职评价表")
    ws.cell(2, 1, "序号"); ws.cell(2, 2, "单位"); ws.cell(2, 3, "岗位"); ws.cell(2, 4, "姓名")
    ws.cell(2, 5, "考核维度（100分）"); ws.cell(2, 30, "得分\n排序")
    # 第 1 项 20、第 7 项 10、第 12 项 10、第 13 项 15，其余各 5 分（列 5,7,...,29）
    names = ["安全绩效", "隐患排查", "安全检查", "安全教育", "危险源管理", "应急管理",
             "违章查处", "机构人员", "制度规程", "费用投入", "履职评价",
             "“六个一”工作实绩", "甲方契合度"]
    scores = [20, 5, 5, 5, 5, 5, 10, 5, 5, 5, 5, 10, 15]
    for j, (col, s) in enumerate(zip(range(5, 32, 2), scores)):
        ws.cell(3, col, "%d分" % s)
        ws.cell(4, col, names[j])
        ws.cell(4, col + 1, "扣分说明")
    ws.cell(5, 1, 1); ws.cell(5, 2, "兴达公司"); ws.cell(5, 3, "事业部主要负责人"); ws.cell(5, 4, "汪庆")
    for col, val in zip(range(5, 32, 2), [20, 5, 5, 5, 5, 5, 8, 5, 5, 5, 5, 10, 15]):
        ws.cell(5, col, val)
    ws.cell(5, 18, "未按违章查处")
    ws.cell(5, 30, 93)
    ws.cell(6, 4, "王旺")
...
def test_read_eval_scores():
    ...
    try:
        d = kc.read_eval_scores(p, "汪庆")
        assert d["name"] == "汪庆"
        assert d["total"] == "93"
        assert d["items"][1]["score"] == "20"
        assert d["items"][3]["score"] == "5"
        assert d["items"][7]["desc"] == "未按违章查处"
        ...
        try:
            ...
            assert "汪庆" in str(e) and "王旺" in str(e)
        # 无目标 sheet：报错
        import openpyxl
...
def test_list_eval_names():
    tmp, p = _make_fake_lvzhilv()
    try:
        assert kc.list_eval_names(p) == ["汪庆", "王旺"]
        # 无目标 sheet 时同样报错
        import openpyxl
        bad = os.path.join(tmp, "其他.xlsx")
        wb = openpyxl.Workbook()
...
def test_month_from_filename():
    ...
    assert kc._month_from_filename("主要负责人安全生产责任制履职清单考评表（汪庆7月）.doc") == "7"
...
def test_parse_kaoping_header():
    hdr = "考评对象（主要负责人）：汪庆  评价月份：7月评价人员签字：（手签）\r\x07"
    name, month = kc._parse_kaoping_header(hdr)
    assert name == "汪庆" and month == "7"
    # 半角冒号 / 缺月份
    name2, month2 = kc._parse_kaoping_header("考评对象（主要负责人）: 王旺 评价月份:")
