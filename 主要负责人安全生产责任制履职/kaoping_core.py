...
TEMPLATE_NAME = "主要责任人安全生产责任制履职清单考评表（模板）.doc"


# 考评项 → 表格主行映射：13 个考评项，每项一个主填写行
# （各主行的填写格与续行纵向合并，填入主行即可覆盖整个考评块；续行 C4/C5 为模板固定内容）。
ITEM_ROWS = {
    1: 3,     # 安全绩效（20，占 R3~R5）
    2: 6,     # 隐患排查（5，占 R6~R8）
    3: 9,     # 安全检查（5）
    4: 10,    # 安全教育（5，占 R10~R11）
    5: 12,    # 危险源管理（5，占 R12~R13）
    6: 14,    # 应急管理（5，占 R14~R15）
    7: 16,    # 违章查处（10，占 R16~R18）
    8: 19,    # 机构人员（5）
    9: 20,    # 制度规程（5，占 R20~R24）
    10: 25,   # 费用投入（5）
    11: 26,   # 履职评价（5，占 R26~R33）
    12: 34,   # “六个一”工作实绩（10，占 R34~R39）
    13: 40,   # 甲方契合度（15）
}
TOTAL_ROW = 41
N_ITEMS = len(ITEM_ROWS)   # 13 个考评项
...
DEFAULT_NAME_PATTERN = "主要负责人安全生产责任制履职清单考评表({X}月{XXX}).doc"

# 表头占位标签：主要负责人模板表头无「管理者姓名」栏
# （结构：考评对象（主要负责人）：姓名 … 评价月份：X月 … 评价人员签字）
HEADER_ROLE_LABEL = "考评对象（主要负责人）："   # _replace_slot 用（含冒号）
HEADER_ROLE_PREFIX = "考评对象（主要负责人）"    # _slot 用（不含冒号，自行定位冒号）
HEADER_ROLE_NEXT = "评价月份"                     # 姓名区间后标签
...
# 关键词基于主要负责人 13 个考评项标题与常见当月文件夹命名归纳（首词权重 2，可编辑 material_rules.json）。
ITEM_MATCH_RULES = {
    1: ["安全绩效", "绩效", "事故", "工伤", "轻伤", "险肇", "目标", "否决", "连带"],
    2: ["隐患排查", "隐患", "排查", "登记", "整改", "扫雷", "闭环", "验证"],
    3: ["安全检查", "检查", "综合检查", "节假日", "巡查", "督查", "整改"],
    4: ["安全教育", "培训", "教育", "档案", "宣传", "资质", "持证", "授课"],
    5: ["危险源管理", "危险源", "风险", "辨识", "控制点", "评估", "管理评审"],
    6: ["应急管理", "应急", "演练", "预案", "物资", "器材", "设备检查"],
    7: ["违章查处", "违章", "查处", "三违", "违规", "违纪", "安监系统", "3+1", "警示"],
    8: ["机构人员", "机构", "人员", "配备", "专职", "安全管理"],
    9: ["制度规程", "制度", "规程", "责任制", "标准化", "岗位规程", "评审", "修订", "职责"],
    10: ["费用投入", "费用", "投入", "资金", "安全条件", "改善"],
    11: ["履职评价", "履职", "责任书", "考评", "挂钩点", "带班", "安委会", "专题会", "计划"],
    12: ["六个一", "六个一", "风险管控评审", "交心", "改善项目", "案例", "行为观察", "授课"],
    13: ["甲方契合度", "甲方", "契合", "武钢有限", "安委会", "专题会", "重点", "反馈"],
}
...
# ============ 月度履职履责表(xlsx) 评分自动读取 ============
# 目标 sheet：「主要负责人月度履职评价表」（关键字匹配，排除「评价标准」）。
# 布局参照安全员版实测规律：表头行为含「扣分说明」的行；偶数列为各考评项名、奇数列为其扣分说明；
# 姓名在「姓名」列；得分合计在含「得分」「合计」的表头列。
# 注：主要负责人履职履责表实际版式需以业务方提供的月度 xlsx 为准（关键词匹配自适应）。
XLSX_SHEET_INCLUDE = ("主要负责人", "评价")
XLSX_SHEET_EXCLUDE = ("标准",)
# 13 个考评项在 xlsx 表头中的匹配关键词（任一命中即可）
...
def _locate_eval_sheet(xlsx_path):
    ...
    for sn in wb.sheetnames:
        if (all(k in sn for k in XLSX_SHEET_INCLUDE)
            ...
            break
    if sheet is None:
        raise ValueError("未找到「主要负责人月度履职评价表」sheet，可用：%s"
                         % "、".join(wb.sheetnames))
...
def _parse_kaoping_header(header_text):
    ...
    name = _slot(body, HEADER_ROLE_PREFIX, HEADER_ROLE_NEXT)
...
def _fill_header(cell, name, month):
    ...
    # 姓名：考评对象（主要负责人）：...评价月份（模板无「管理者姓名」栏）
...
def _sum_scores(items, key):
    """对某字段的数值求和（用于合计），忽略非数值；双行项第 2 行一并计入；无有效数值返回空。"""
    total = 0
    has_val = False
    for idx in range(1, N_ITEMS + 1):
...
def generate_doc(template_path, output_path, name, month, items, year=None,
    ...
    items: {1..13: {'desc','score','material_text','material_images':[path],
    ...
    try:
        ...
        if "考评对象" not in header or "评价月份" not in header:
            raise ValueError(
                "模板表头缺少「考评对象（主要负责人）」或「评价月份」标签，请更换正确模板"
            )
