...
def test_app_build_layout():
    ...
    try:
        root, app = _new_app(tmp)
        try:
            assert len(app.items_widgets) == 17
...
def test_validate_items():
    ...
    try:
        root, app = _new_app(tmp)
        try:
            ok = {i: {"score": "", "super_score": ""} for i in range(1, 18)}
...
def test_material_rules_and_weights():
    ...
    try:
        ...
        assert kc.match_materials_file("（炼铁）8月主题工作/标准化作业.xlsx") == 16
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                assert "17 项" in app.status_var.get()
            finally:
                root.destroy()
        finally:
...
def test_eval_scores_fill():
    ...
    try:
        ...
        kc.list_eval_names = lambda p: ["季忠", "王旺"]
        kc.read_eval_scores = lambda p, name: {
            "name": name, "total": "93",
            "items": {i: {"score": "5", "desc": "描述%d" % i} for i in range(1, 18)},
        }
        try:
            root, app = _new_app(tmp)
            try:
