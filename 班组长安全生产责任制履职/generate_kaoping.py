#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""班组长安全生产责任制履职清单考评表 自动生成工具 GUI v2.0.0（兼容 Windows 7 SP1 ~ Windows 11）"""
import logging
import os
import sys
import json
...
class App:
    def __init__(self, root):
        ...
        self.items_widgets = []
        root.title(f"班组长安全生产责任制履职清单考评表自动生成工具 v{VERSION}")
    ...
    def _build_ui(self):
        ...
        ToolTip(self.eval_label,
                "读取月度履职履责表（.xlsx）：\n"
                f"按「班组长月度履职评价表」sheet 读取所选班组长的 {len(ITEM_LABELS)} 项评分（excel 只提供姓名和分数），\n"
                "自动填入各考评项的「自评得分」。\n"
                "文件中包含多个姓名时，会弹出对话框让您选择本次生成的班组长。")
    ...
    def _pick_eval_name(self, names):
        ...
        ttk.Label(win, text="履职履责表中包含多个班组长，请选择本次生成的姓名：",
                  padding=(14, 10)).pack()
